...
const Hero = () => {
  ...
  useEffect(() => {
    // ตรวจสอบว่าถ้าต้องการให้เล่นแค่ครั้งเดียว ให้ป้องกันการ loop
    // (แต่ถ้าเปิด loop อยู่ อย่าลืมเอาออก)
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* Dark overlay: แสดงหลังวิดีโอจบ */}
      <div
        className={`absolute inset-0 bg-black transition-opacity duration-1000 z-10 ${
          videoEnded ? 'opacity-50' : 'opacity-0'
        }`}
      ></div>

      {/* Animated blobs: แสดงหลังวิดีโอจบ */}
      {videoEnded && (
      ...
      {/* Content: แสดงหลังวิดีโอจบ พร้อมอนิเมชัน fade-in */}
      <div
      ...
      >
        <div className="text-left md:text-center text-white drop-shadow-xl">
          ...
          <p className="md:block text-base md:text-xl text-gray-100 mb-10 max-w-3xl mx-auto leading-relaxed text-balance">
            เราให้บริการโซลูชันเทคโนโลยีล้ำสมัย ที่ช่วยขับเคลื่อนนวัตกรรม ยกระดับประสิทธิภาพ
            และเร่งการเปลี่ยนผ่านสู่ยุคดิจิทัลของคุณให้ก้าวไกลยิ่งขึ้น
          </p>
